...
export class ProgressListStyles {
  /**
   * Get progress list style based on props
   */
  static getStyle(props: ProgressListProps) {
   ...
   */
  private static getVariantStyle(variant: ProgressListVariants) {
    switch (variant) {
      case 'primary':
        return {
   ...
   */
  private static getSizeStyle(size: ProgressListSizes) {
    switch (size) {
      case 'xs':
        return {
          padding: {
   ...
   */
  private static getStateStyle(state: ProgressListStates) {
    switch (state) {
      case 'hover':
        return {
  ...
  /**
   * Get custom styles based on progress list props
   */
  private static getCustomStyle(props: ProgressListProps) {
    ...
    // Progress list style variants
    if (props.progressListStyle === 'compact') {
      customStyle.padding = {
        left: 2,
        right: 2,
        top: 2,
    ...
    if (props.progressListStyle === 'detailed') {
      customStyle.padding = {
        left: 6,
        right: 6,
        top: 4,
    ...
    if (props.progressListStyle === 'minimal') {
      customStyle.padding = {
        left: 1,
        right: 1,
        top: 1,
    ...
    if (props.progressListStyle === 'modern') {
      customStyle.border = {
        type: 'line',
        fg: tokens.colors.primary[400],
        bg: this.getCurrentBg(),
  ...
  /**
   * Get progress list style for specific variant/size/state combination
   */
  static getStyleForCombination(
    ...
    const mockProps: ProgressListProps = {
      variant,
      size,
      state,
    };
  ...
  /**
   * Get all available progress list styles
   */
  static getAllStyles() {
    const variants: ProgressListVariants[] = ['default', 'primary', 'secondary', 'success', 'warning', 'error', 'info', 'outline', 'ghost', 'striped', 'animated', 'gradient'];
    const sizes: ProgressListSizes[] = ['xs', 'sm', 'md', 'lg', 'xl'];
    const states: ProgressListStates[] = ['default', 'hover', 'active', 'disabled', 'focus', 'error', 'success', 'loading', 'complete', 'indeterminate'];
