...
export class RadioGroupStyles {
  /**
   * Get radio group style based on props
   */
  static getStyle(props: RadioGroupProps) {
   ...
   */
  private static getVariantStyle(variant: RadioGroupVariants) {
    switch (variant) {
      case 'primary':
        return {
   ...
   */
  private static getSizeStyle(size: RadioGroupSizes) {
    switch (size) {
      case 'xs':
        return {
          padding: {
   ...
   */
  private static getStateStyle(state: RadioGroupStates) {
    switch (state) {
      case 'hover':
        return {
  ...
  /**
   * Get custom styles based on radio group props
   */
  private static getCustomStyle(props: RadioGroupProps) {
    ...
    if (props.radioGroupStyle === 'compact') {
      customStyle.padding = {
        left: 2,
        right: 2,
        top: 2,
    ...
    if (props.radioGroupStyle === 'detailed') {
      customStyle.padding = {
        left: 6,
        right: 6,
        top: 4,
    ...
    if (props.radioGroupStyle === 'minimal') {
      customStyle.padding = {
        left: 1,
        right: 1,
        top: 1,
    ...
    if (props.radioGroupStyle === 'modern') {
      customStyle.border = {
        type: 'line',
        fg: tokens.colors.primary[400],
        bg: this.getCurrentBg(),
  ...
  /**
   * Get radio group style for specific variant/size/state combination
   */
  ...
  /**
   * Get all available radio group styles
   */
  static getAllStyles() {
    const variants: RadioGroupVariants[] = ['default', 'primary', 'secondary', 'success', 'warning', 'error', 'info', 'outline', 'ghost', 'striped', 'animated', 'gradient'];
    const sizes: RadioGroupSizes[] = ['xs', 'sm', 'md', 'lg', 'xl'];
    const states: RadioGroupStates[] = ['default', 'hover', 'active', 'disabled', 'focus', 'error', 'success', 'loading', 'complete', 'indeterminate'];
