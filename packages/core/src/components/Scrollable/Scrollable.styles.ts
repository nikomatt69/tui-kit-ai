...
export class ScrollableStyles {
  /**
   * Get scrollable style based on props
   */
  static getStyle(props: ScrollableProps) {
   ...
   */
  private static getVariantStyle(variant: ScrollableVariants) {
    switch (variant) {
      case 'primary':
        return {
   ...
   */
  private static getSizeStyle(size: ScrollableSizes) {
    switch (size) {
      case 'xs':
        return {
          padding: {
   ...
   */
  private static getStateStyle(state: ScrollableStates) {
    switch (state) {
      case 'hover':
        return {
  ...
  /**
   * Get custom styles based on scrollable props
   */
  private static getCustomStyle(props: ScrollableProps) {
    ...
    // Scrollable style variants
    if (props.scrollableStyle === 'compact') {
      customStyle.padding = {
        left: 2,
        right: 2,
        top: 2,
    ...
    if (props.scrollableStyle === 'detailed') {
      customStyle.padding = {
        left: 6,
        right: 6,
        top: 4,
    ...
    if (props.scrollableStyle === 'minimal') {
      customStyle.padding = {
        left: 1,
        right: 1,
        top: 1,
    ...
    if (props.scrollableStyle === 'modern') {
      customStyle.border = {
        type: 'line',
        fg: tokens.colors.primary[400],
        bg: this.getCurrentBg(),
  ...
  /**
   * Get scrollable style for specific variant/size/state combination
   */
  static getStyleForCombination(
    ...
    const mockProps: ScrollableProps = {
      variant,
      size,
      state,
    };
  ...
  /**
   * Get all available scrollable styles
   */
  static getAllStyles() {
    const variants: ScrollableVariants[] = ['default', 'primary', 'secondary', 'success', 'warning', 'error', 'info', 'outline', 'ghost', 'striped', 'animated', 'gradient'];
    const sizes: ScrollableSizes[] = ['xs', 'sm', 'md', 'lg', 'xl'];
    const states: ScrollableStates[] = ['default', 'hover', 'active', 'disabled', 'focus', 'error', 'success', 'loading', 'complete', 'indeterminate'];
