import { TreeProps, TreeVariants, TreeSizes, TreeStates } from './Tree.types';
import { tokens, semanticColors } from '../../theming/tokens';

export class TreeStyles {
  /**
   * Get tree style based on props
   */
  static getStyle(props: TreeProps) {
   ...
   */
  private static getVariantStyle(variant: TreeVariants) {
    switch (variant) {
      case 'primary':
        return {
          bg: tokens.colors.white,
   ...
   */
  private static getSizeStyle(size: TreeSizes) {
    switch (size) {
      case 'xs':
        return {
          padding: {
   ...
   */
  private static getStateStyle(state: TreeStates) {
    switch (state) {
      case 'hover':
        return {
  ...
  /**
   * Get custom styles based on tree props
   */
  private static getCustomStyle(props: TreeProps) {
  ...
  /**
   * Get tree style for specific variant/size/state combination
   */
  ...
  /**
   * Get all available tree styles
   */
  static getAllStyles() {
    const variants: TreeVariants[] = ['default', 'primary', 'secondary', 'success', 'warning', 'error', 'info', 'outline', 'ghost', 'striped', 'animated', 'gradient'];
    const sizes: TreeSizes[] = ['xs', 'sm', 'md', 'lg', 'xl'];
    const states: TreeStates[] = ['default', 'hover', 'active', 'disabled', 'focus', 'error', 'success', 'loading', 'complete', 'indeterminate'];
