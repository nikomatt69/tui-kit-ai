...
export class StepperStyles {
  /**
   * Get stepper style based on props
   */
  static getStyle(props: StepperProps) {
   ...
   */
  private static getVariantStyle(variant: StepperVariants) {
    switch (variant) {
      case 'primary':
        return {
   ...
   */
  private static getSizeStyle(size: StepperSizes) {
    switch (size) {
      case 'xs':
        return {
          padding: {
   ...
   */
  private static getStateStyle(state: StepperStates) {
    switch (state) {
      case 'hover':
        return {
  ...
  /**
   * Get custom styles based on stepper props
   */
  private static getCustomStyle(props: StepperProps) {
    ...
    // Stepper style variants
    if (props.stepperStyle === 'compact') {
      customStyle.padding = {
        left: 1,
        right: 1,
        top: 1,
    ...
    if (props.stepperStyle === 'detailed') {
      customStyle.padding = {
        left: 4,
        right: 4,
        top: 3,
  ...
  /**
   * Get stepper style for specific variant/size/state combination
   */
  ...
  /**
   * Get all available stepper styles
   */
  static getAllStyles() {
    const variants: StepperVariants[] = ['default', 'primary', 'secondary', 'success', 'warning', 'error', 'info', 'outline', 'ghost', 'striped', 'animated', 'gradient'];
    const sizes: StepperSizes[] = ['xs', 'sm', 'md', 'lg', 'xl'];
    const states: StepperStates[] = ['default', 'hover', 'active', 'disabled', 'focus', 'error', 'success', 'loading', 'complete', 'indeterminate'];
